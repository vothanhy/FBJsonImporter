...
class FBJsonImporter {
	...
	public static function import_json($file) {
		...
		//generate test json data files
		/*
		for($i=0;$i<49975;$i++) {
			$arr->data[] = clone $arr->data[0];
			$id = $arr->data[count($arr->data)-1]->id;
			$r = rand(10000,90000);
			$id = substr($id."",0,strlen($id."")-7) . $r;
			$arr->data[count($arr->data)-1]->id = $id;
			
			if(isset($arr->data[count($arr->data)-1]->name)) {
				$arr->data[count($arr->data)-1]->name .= " " .$r;
			} else {
				if(isset($arr->data[count($arr->data)-1]->story)) { 
					$arr->data[count($arr->data)-1]->story = $arr->data[count($arr->data)-1]->story . " " . $r;
				}
			}
		}
		file_put_contents("C:\\xampp\\htdocs\\wp42\\wp-content\\plugins\\FBJsonImporter\\json50000.json",json_encode($arr));
		//print_r($arr->data);
		return;
		*/
...
function FBJsonImporter_shortcode() {
